  ...
  return (
    <div className="relative w-full bg-[var(--color-background)] px-5 lg:px-10">
      <div className="flex items-center justify-between h-[64px] container mx-auto">
        <Link to="/" className="text-3xl lg:text-5xl text-white cursor-pointer hover:text-[var(--color-secondary)] duration-300">
          FilmSearch
        </Link>

        {/* Pasek wyszukiwania (desktop) */}
        <label
              ...
              {searchResults.map(item => (
                <li key={item.id}>
                  <Link
                    ...
                    {item.img && <img src={item.img} alt={item.title || item.name} className="w-10 h-14 object-cover rounded" />}
                    <span>
        ...
        <div className="hidden xl:flex items-center gap-10">
          <div className="flex gap-5 text-white text-lg font-light">
            <button onClick={() => changeType('movie')} className="flex items-center space-x-1 transition cursor-pointer hover:text-[var(--color-secondary)] duration-150 hover:scale-105">
              <FaFilm />
              <span>Movies</span>
            </button>
            <button onClick={() => changeType('tv')} className="flex items-center space-x-1 transition cursor-pointer hover:text-[var(--color-secondary)] duration-150 hover:scale-105">
              <FaTv />
              <span>TV Shows</span>
            </button>
          </div>
          <div className="flex gap-5 text-white">
            <a href="#" className="bg-[var(--color-primary)] hover:bg-[var(--color-secondary)] px-4 py-2 rounded-xl hover:text-black duration-300">
              Sign Up
            </a>
            <a href="#" className="ring-2 ring-[var(--color-primary)] hover:bg-[var(--color-primary)] px-4 py-2 rounded-xl duration-300">
              Log In
            </a>
          </div>
        </div>
      ...
      <div
        ...
        <button onClick={() => changeType('movie')} className="text-white text-xl py-2 w-full hover:text-[var(--color-secondary)] flex items-center justify-center gap-3">
          <FaFilm /> Movies
        </button>
        <hr />
        <button onClick={() => changeType('tv')} className="text-white text-xl py-2 w-full hover:text-[var(--color-secondary)] flex items-center justify-center gap-3">
          <FaTv /> TV Shows
        </button>
      </div>

      {/* Dropdown autoryzacji (mobilny) */}
      <div
        className={classNames(
        ...
        <a href="#" className="text-white text-lg py-2 hover:text-[var(--color-secondary)] flex items-center justify-center gap-3">
          Sign Up
        </a>
        <hr />
        <a href="#" className="text-white text-lg py-2 hover:text-[var(--color-secondary)] flex items-center justify-center gap-3">
